        ...
        <div className={`alert-container ${show ? "show" : "hide"}`}>
            <div className={`alert alert-${type}`}>
                <span className="alert-icon">
                    {type === AlertType.SUCCESS ? <CircleCheckBig className="size-[16px]" /> : null}
                    {type === AlertType.ERROR ? <TriangleAlert className="size-[16px]" /> : null}
                    {type === AlertType.INFO ? <Info className="size-[16px]" /> : null}
                    {type === AlertType.WARNING ? <CircleAlert className="size-[16px]" /> : null}
                </span>
